...
public class BadgeReaderSensorProbe extends Service implements
	...
	private void sendData(final JsonObject data) {
		...
		double value = Double.parseDouble(data.get("distance").getAsString());
		int badgeEventValue = Double.compare(value, 0);

		if (badgeEventValue > 0) {

			pubSubSensingFramework.publish(BadgeDetectedEvent, data);

		} else {
			pubSubSensingFramework.publish(BadgeDisappearedEvent, data);
		}
